    ...
    public function addRandom($number, $table)
    {
        ...
        while ($number--) {
            foreach ($this->_fields[$table] as $key => $value) {
                ...
                } elseif ('varchar' === $value[0]) {
                    $v = $this->randomVarchar($value[1]);
                } elseif ('text' === $value[0]) {
                    $v = $this->randomVarchar();
                } elseif ('int' === $value[0] and '11' == $value[1]) {
                    $v = rand(1, 2);
                } elseif ('int' === $value[0] and '10' == $value[1]) {
                    $v = rand(1, 255);
                } elseif ('datetime' === $value[0]) {
                    $v = $this->randomDatatime();
                }
